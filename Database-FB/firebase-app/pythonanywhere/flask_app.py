...
app = Flask(__name__)
pageName = 'firebase_api'

# File Dir
basepath = os.path.abspath(".")
file_path = os.path.join(basepath, pageName,'serviceAccountKey.json') # credential keys

localEnv = False # Production env flag

# Check project environment to set Firebase Client
if not localEnv:
    # production
    cred = credentials.Certificate(file_path) # use absolute paths https://help.pythonanywhere.com/pages/NoSuchFileOrDirectory/
...
if __name__ == '__main__':
    if not localEnv: #os.getenv('GAE_ENV', '').startswith('standard'):
        app.run()  # production
    else:
        # if localEnv
